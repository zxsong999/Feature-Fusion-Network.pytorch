...
def prepare_folders_classifier(args):
    time_str = time.strftime('%Y%m%d%H%M')
    store_name = '_'.join([args.dataset, args.loss_type, str(args.lam), args.train_rule, str(args.imb_factor),
                           args.exp_str, time_str])
    store_name = os.path.join('saved', store_name)
...
def plot_norms(model, labelnames, y_range=None):
    ...
    plt.savefig(r"C:/Users/song/Desktop/DFFN/" + "DFFN_norm" + ".pdf", format='pdf')
    plt.close()
