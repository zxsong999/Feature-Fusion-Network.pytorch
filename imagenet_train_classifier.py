...
from losses import *
from sampler import *
from dataset.imbalance_data.lt_data import LT_Dataset
model_names = sorted(name for name in models.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(models.__dict__[name]))

parser = argparse.ArgumentParser(description='PyTorch imagenet Training')
parser.add_argument('--root', default='', help='dataset setting')
parser.add_argument('--dataset', default='Imagenet-LT', help='dataset setting')
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet50', choices=model_names,
                    help='model architecture: ' + ' | '.join(model_names) + ' (default: resnet50)')
parser.add_argument('--loss_type', default="LDAM", type=str, help='loss type')  # Noise LDAM L2cons
parser.add_argument('--imb_type', default="exp", type=str, help='imbalance type')
parser.add_argument('--imb_factor', default=None, type=float, help='imbalance factor')
parser.add_argument('--train_rule', default='BalancedRS', type=str,
                    help='data sampling strategy for train loader')  # EffectNumRS  ClassAware
...
parser.add_argument('--resume_KPS', default='', type=str, metavar='PATH')  # L2cons  long-tailed
parser.add_argument('--resume_CE', default='', type=str, metavar='PATH')
parser.add_argument('-e', '--evaluate', dest='evaluate', action='store_true',
                    help='evaluate model on validation set')
parser.add_argument('--seed', default=666, type=int,  # None
...
def main():
    ...
    args.store_name = prepare_folders_classifier(args)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.loss_type == 'Noise':
        ...
        use_norm = True
        use_noise = False
    model_KPS = models.__dict__[args.arch](num_classes=num_classes, classifier=False,
                                           use_norm=use_norm, use_noise=use_noise)
    model_CE = models.__dict__[args.arch](num_classes=num_classes, classifier=False,
                                          use_norm=use_norm, use_noise=use_noise)
    # get classifier
    ...
    train_dataset = LT_Dataset(args.root, './dataset/ImageNet_LT/ImageNet_LT_train.txt', transform_train)
    val_dataset = LT_Dataset(args.root, './dataset/ImageNet_LT/ImageNet_LT_test.txt', transform_val)
    ...
    # balanced model
    if os.path.isfile(args.resume_KPS):
        print("=> loading checkpoint '{}'".format(args.resume_KPS))
        # load model
        model_dict_KPS = model_KPS.state_dict()
        checkpoint_KPS = torch.load(args.resume_KPS, map_location='cuda:0')
        # 1. filter out unnecessary keys
        pretrained_dict_KPS = {k: v for k, v in checkpoint_KPS['state_dict'].items() if k in model_dict_KPS}
        # 2. overwrite entries in the existing state dict
        model_dict_KPS.update(pretrained_dict_KPS)
        model_KPS.load_state_dict(model_dict_KPS)
        print("=> loading checkpoint '{}'".format(args.resume_CE))
        # load model_2
        model_dict_CE = model_CE.state_dict()
        checkpoint_CE = torch.load(args.resume_CE, map_location='cuda:0')
        # 1. filter out unnecessary keys
        pretrained_dict_CE = {k: v for k, v in checkpoint_CE['state_dict'].items() if k in model_dict_CE}
        # 2. overwrite entries in the existing state dict
        model_dict_CE.update(pretrained_dict_CE)
        model_CE.load_state_dict(model_dict_CE)

        # load classifier
        model_dict_ = classifier.state_dict()
        model_dict_['module.weight'] = checkpoint_KPS['state_dict']['module.linear.weight']
        if use_norm == False:
            model_dict_['module.bias'] = checkpoint_KPS['state_dict']['module.linear.bias']
        classifier.load_state_dict(model_dict_)
    else:
        print("=> no checkpoint found at '{}'".format(args.resume))

    for name, param in model_KPS.named_parameters():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model_KPS, model_CE, classifier, lws_model, criterion, optimizer, epoch, args, log_training,
              tf_writer)

        # evaluate on validation set
        acc1, val_loss = validate(val_loader, model_KPS, model_CE, classifier, lws_model, criterion, epoch, args,
...
def train(train_loader, model_KPS, model_CE, classifier, lws_model, criterion, optimizer, epoch, args, log, tf_writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.mixup is True:
            ...
            feature_1 = model_KPS(images, get_feat=True)
            feature_2 = model_CE(images, get_feat=True)
            output_1 = classifier(feature_1.detach())
            output_2 = classifier(feature_2.detach())
            output = output_1 * args.lam + output_2 * (1 - args.lam)
            ...
            feature_1 = model_KPS(input, get_feat=True)
            feature_2 = model_CE(input, get_feat=True)
            output_1 = classifier(feature_1.detach())
            output_2 = classifier(feature_2.detach())
            output = output_1 * args.lam + output_2 * (1 - args.lam)
            if args.loss_type == 'Noise':
...
def validate(val_loader, model_KPS, model_CE, classifier, lws_model, criterion, epoch, args, log=None, tf_writer=None,
             flag='val'):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
